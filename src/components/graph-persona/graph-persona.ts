...
export class PersonaComponent extends LitElement {
    ...
    renderImage() {
        if (this.user) {
            if (this.profileImage) {
                return html`<img class="persona-user-image" src=${this.profileImage as string}></img>`
            } else {
                return html`
    ...
    renderEmptyImage() {
        return html`
            <svg width=${this.imageSize} height=${this.imageSize} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
